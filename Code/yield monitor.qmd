...
```{r}
sy <- read_sf("../Output/yield_interpolated.geojson") %>% 
  mutate(medianyield_lbac = median(ipyield_lbac)) %>% 
  mutate(sy_median = round(ipyield_lbac/medianyield_lbac * 100, 1)) %>% 
  dplyr::select(sy_median, geometry)
...
sy_class <- sy %>% 
  mutate(stclass = case_when(
    sy_median < 88.7 ~ "low",
    sy_median >=88.7 & sy_median <=105.2 ~ "medium",
    sy_median > 105.2 ~ "high"
  ))
